...
import Date.Extra.Facts exposing (daysInMonth, monthFromMonthNumber, weekdayNumberFromDayOfWeek, msPerSecond, msPerMinute, msPerHour, msPerDay)
...
add interval n date =
  ...
  in
    case interval of
      Millisecond -> Date.fromTime <| toTime date + (toFloat n)
      Second      -> Date.fromTime <| toTime date + (toFloat <| n * msPerSecond)
      Minute      -> Date.fromTime <| toTime date + (toFloat <| n * msPerMinute)
      Hour        -> Date.fromTime <| toTime date + (toFloat <| n * msPerHour)
